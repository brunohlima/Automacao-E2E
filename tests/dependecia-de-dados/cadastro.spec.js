...
test('Nível 2 - Dependência de Dados: Criar Departamento e Usuário Vinculado', async ({ page }) => {
  ...
  await page.goto('https://qa.example.invalid/');
  
  await page.getByTestId('login-input-email').fill('usuario@example.invalid');
  await page.getByTestId('login-input-password').fill('REDACTED_PASSWORD');
  ...
  await page.getByTestId('users-form-input-password').fill('REDACTED_PASSWORD');
  await page.getByTestId('users-form-input-passwordConfirmation').fill('REDACTED_PASSWORD');
